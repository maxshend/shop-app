# frozen_string_literal: true

require 'rails_helper'

RSpec.describe 'DELETE /api/admin/products/:id', type: :request do
  context 'when product found' do
    let!(:product) { create :product }

    context 'when product can be destroyed' do
      before :each, with_before: true do
        delete "/api/admin/products/#{product.id}"
      end

      it 'has ok status', :with_before do
        expect(response).to have_http_status :ok
      end

      it 'returns success', :with_before do
        expect(json).to eq 'success' => true
      end

      it 'destroys product record' do
        expect do
          delete "/api/admin/products/#{product.id}"
        end.to change(Product, :count).from(1).to 0
      end
    end

    context 'when product cannot be destroyed' do
      let(:error_message) { 'Cannot be destroyed' }
      let(:destroy_product) { instance_double(Product) }

      before do
        allow(Product).to receive(:find_by).and_return destroy_product
        allow(destroy_product).to receive(:destroy).and_return false
        fake_error destroy_product, :base, error_message

        delete "/api/admin/products/#{product.id}"
      end

      it 'has unprocessable status' do
        expect(response).to have_http_status :unprocessable_entity
      end

      it 'returns errors' do
        expect(json).to eq 'errors' => [{ 'key' => 'base', 'messages' => [error_message] }]
      end
    end
  end

  context 'when product not found' do
    before do
      delete '/api/admin/products/0'
    end

    it 'has unprocessable status' do
      expect(response).to have_http_status :unprocessable_entity
    end

    it 'returns errors' do
      expect(json).to eq 'errors' => [{ 'key' => 'base', 'messages' => ['Product not found'] }]
    end
  end
end
